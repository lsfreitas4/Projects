  ...
  return (Store<AppState> store) async {
    try {
      ...
      if(username =='TestUser' && password=='TestPassword'){
          final Session session = Session(authenticated: true, studentNumber: 'up202000000');
          Logger().i('Login successful');
          store.dispatch(SaveLoginDataAction(session));
          store.dispatch(SetLoginStatusAction(RequestStatus.successful));

          final profile = await NetworkRouter.getProfile(store.state.content['session']);
          ...
          store.dispatch( UpdateFavoriteCards([]));
          store.dispatch(SetExamFilter({}));
