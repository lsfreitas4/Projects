...
export class ShaderScene extends CGFscene {
	...
	init(application) {
		...
		this.testShaders = [
			...
			new CGFshader(this.gl, "shaders/positionColor.vert", "shaders/positionColor.frag"),
		...
		this.shadersList = {
			...
			'Sepia': 7,
			'Convolution': 8,
			'Teapot by clip-space position': 9,
			'Grayscale': 10,
			'Water effect': 11
		};
	...
	update(t) {
		if (this.selectedExampleShader == 6)
			// Keep the original behavior for the provided animation example.
			this.testShaders[6].setUniformsValues({ timeFactor: t / 100 % 100 });

		if (this.selectedExampleShader == 11)
			// Keep it bounded to avoid precision artifacts in fract() texture animation.
			this.testShaders[11].setUniformsValues({ timeFactor: t / 500 % 100.0 });
	}
	...
	display() {
		...
		// apply main appearance with default texture on unit 0
		if (this.selectedExampleShader == 11)
