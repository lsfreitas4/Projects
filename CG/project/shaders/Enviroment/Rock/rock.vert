...
void main() {
    ...
    vNoise = noise(aVertexPosition.xz * 2.5 + vec2(uRockSeed));
    float perturbation = vNoise * 0.45;
    vec3 perturbedPosition = aVertexPosition + aVertexNormal * perturbation;

    vec2 terrainUV = vec2(
        uRockWorldXZ.x / uTerrainWidth + 0.5,
        uRockWorldXZ.y / uTerrainWidth + 0.5
    );

    float prairie  = fbm(terrainUV * 4.0);
    float road     = roadMask(terrainUV);
    float roadH    = roadHeightAt(terrainUV);
    float terrainY = mix(prairie, roadH, road) * uTerrainScale;

    float scaleY = length(uMVMatrix[1].xyz);
    float lift   = (scaleY > 0.00001) ? (terrainY / scaleY) : terrainY;

    perturbedPosition.y += lift;

    gl_Position = uPMatrix * uMVMatrix * vec4(perturbedPosition, 1.0);
}
