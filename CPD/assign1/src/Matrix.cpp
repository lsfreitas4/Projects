...
void OnMultLine(int m_ar, int m_br) 
{
    ...
    // Initialize result matrix phc to zero
    for(i = 0; i < m_ar; i++)
        for(j = 0; j < m_br; j++)
            phc[i*m_ar + j] = 0.0;
...
void ParallelMult1(int m_ar, int m_br) 
{
    double Time1, Time2;
...
void ParallelMult2(int m_ar, int m_br) 
{
    double Time1, Time2;
    char st[100];
    int i, j, k;
    ...
    #pragma omp parallel for private(j, k) shared(pha, phb, phc) 
    for(i = 0; i < m_ar; i++) {
        for(k = 0; k < m_ar; k++) {
			#pragma omp 
            for(j = 0; j < m_br; j++) {
	...
	do {
		...
		switch (op){
			...
			case 3:
				...
				break;
			case 4: 
				ParallelMult1(lin, col);
				break;
            case 5: 
				ParallelMult2(lin, col);
				break;
