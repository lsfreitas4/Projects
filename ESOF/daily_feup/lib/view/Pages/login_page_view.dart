...
import 'package:uni/view/Pages/topics_lists_page.dart';
...
class _LoginPageViewState extends State<LoginPageView> {
  ...
  List<Widget> getWidgets(BuildContext context, MediaQueryData queryData) {
    ...
    widgets.add(createContinueWithoutLogInButton(queryData, context));
    widgets.add(
        Padding(padding: EdgeInsets.only(bottom: queryData.size.height / 35)));
  ...
  /// Creates the widget for the user to confirm that he doesn't want to login
  Widget createContinueWithoutLogInButton(queryData, BuildContext context) {
    return Padding(
      padding: EdgeInsets.only(
          left: queryData.size.width / 7, right: queryData.size.width / 7),
      child: SizedBox(
        ...
        child: ElevatedButton(
          key: Key('ContinueWithoutLoggingIn'),
          style: ElevatedButton.styleFrom(
            shape: RoundedRectangleBorder(
              borderRadius: BorderRadius.circular(25),
            ),
          ...
          onPressed: () {
            ...
            Navigator.push(context, MaterialPageRoute(builder: (context) =>  ListaTopico()));
          },
